...
const BinarySearch = () => {
    ...
    const binarySearchAlgo = async () => {
        ...
        setIsSearching(true);
        setCurrentStep("Sorting the array for Binary Search...");
        
        // Binary search requires sorted array, so we sort by value while preserving IDs for animation
        ...
        let arr = sortedArray.map(item => ({ ...item, state: 'idle' }));
        ...
        while (upper >= lower) {
            ...
            setCurrentStep(`Checking middle index ${mid} with value ${arr[mid].value}`);
            await new Promise((resolve) => setTimeout(resolve, 1000));

            if (arr[mid].value === val) {
                setCurrentStep(`Found value ${val} at index ${mid}!`);
                arr[mid].state = 'sorted'; // Using 'sorted' class to signify 'found' (green)
                setArray([...arr]);
                found = true;
                await new Promise((resolve) => setTimeout(resolve, 1000));
                break;
            ...
            if (arr[mid].value > val) {
                setCurrentStep(`Value ${val} < ${arr[mid].value}. Updating upper bound to ${mid - 1}`);
                upper = mid - 1;
            } else {
                setCurrentStep(`Value ${val} > ${arr[mid].value}. Updating lower bound to ${mid + 1}`);
                lower = mid + 1;
            }
            await new Promise((resolve) => setTimeout(resolve, 1000));
        }
        ...
        if (localStorage.getItem("userInfo")) {
            try {
                ...
                await axios.post(
                    ...
                    { userID: userInfo.userID, topic: "binarySearch", completed: true },
            ...
            <div className="pdp-container">
                ...
                <div className="pdp-config">
                    ...
                    <div className="config-header">
                        ...
                        <h1 className="config-title">Binary Search</h1>
                        <div className="config-rating">
                            ★★★★★ <span style={{ color: "var(--text-muted)" }}>(1,240 reviews)</span>
                        </div>
                    ...
                    <div className="config-section">
                        ...
                        <p className="config-theory">
                            Binary Search is an ultra-efficient search algorithm with an O(log n) time complexity. 
                            Operating on the principle of divide and conquer, it requires a sorted array to function. 
                            It repeatedly divides the search interval in half, dramatically narrowing down 
                            the potential location of the target value. Perfect for massive datasets where 
                            linear scanning would be too slow!
                        </p>
                    </div>
